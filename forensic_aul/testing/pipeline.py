...
handler (``launcher/cmds/validate_cmd.py``) only parses arguments and calls
:func:`run`.
...
def _print_devices_or_help() -> int:
    ...
    if not is_macos():
        log.error("Nothing to do. Try `forensic-aul validate --help`.")
        return 1
    try:
...
def _extract_logarchive(
    ...
    run_extract(
        ...
        exhibit_number=None,
        analyst_name=None,
        notes="auto-generated by forensic-aul validate",
