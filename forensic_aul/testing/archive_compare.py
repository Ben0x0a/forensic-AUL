...
Used by : forensic_aul.testing.pipeline (the ``validate --acquisition`` mode).
...
file:
  ...
  differ, shorter is a byte-prefix    → append (the live tail grew between the two
                                        collections on a running device) — expected
  differ, not a prefix                → diverged (a file was rewritten) — a real
                                        acquisition discrepancy
  present in only one archive         → only_a / only_b (rotation: a new current
                                        file is expected; a vanished file is flagged)

Pass = no ``diverged`` files. Everything is streamed/hashed on disk, so a
multi-GB archive costs flat memory.
"""
...
def _is_log_data(rel: str) -> bool:
    return rel.endswith(_LOG_DATA_SUFFIXES) or rel.startswith(_LOG_DATA_DIRS)
...
class ArchiveComparison:
    ...
    def passed(self) -> bool:
        """The acquisitions agree when no shared file was rewritten (identical or
        append-only). Rotation (only_a/only_b) is reported but does not fail."""
...
def _shorter_is_prefix(a: Path, b: Path) -> bool:
    """True when the shorter of *a*/*b* is a byte-prefix of the longer — i.e. the
    longer only *appended* to it. Streamed in chunks (flat memory)."""
    sa, sb = _safe_size(a), _safe_size(b)
    if sa is None or sb is None:
        return False
    short, long = (a, b) if sa <= sb else (b, a)
    remaining = min(sa, sb)
    ...
    for rel in sorted(set(data_a) | set(data_b)):
        ...
        if in_a and in_b:
            if data_a[rel] == data_b[rel]:
                ...
                status = APPEND if _shorter_is_prefix(pa, pb) else DIVERGED
        else:
...
def render_archive_report(cmp: ArchiveComparison) -> str:
    ...
    lines.append("")
    for label, verdicts in (("DIVERGED", cmp.diverged), (f"ONLY IN {cmp.label_a}", cmp.only_a)):
        if verdicts:
